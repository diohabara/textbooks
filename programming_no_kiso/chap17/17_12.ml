...
(* objective : insert kiten, shuten, and kyori into ekikan_tree *)
(* isnert : ekikan_tree_t -> string -> string -> float -> ekikan_tree_t *)
let rec insert ekikan_tree kiten shuten kyori = match ekikan_tree with
    Empty -> Node (Empty, kiten, [(shuten, kyori)], Empty)
  | Node (left, ekimei, lst, right) ->
     if  kiten < ekimei then Node (insert left kiten shuten kyori, ekimei, lst, right)
     else if ekimei < kiten then Node (left, ekimei, lst, insert right kiten shuten kyori)
     else Node (left, ekimei, (shuten, kyori) :: lst, right)

(* objective : insert ekikan into ekikan_tree *)
(* isnert_ekikan : ekikan_tree_t -> ekikan_t -> ekikan_tree_t *)
let insert_ekikan ekikan_tree ekikan = match ekikan with
    {kiten =k; shuten = s; keiyu = y; kyori = r; jikan = j} ->
    insert (insert ekikan_tree s k r) k s r;;
