...
v    {kanji="日比谷"; kana="ひびや"; romaji="hibiya"; shozoku="千代田線"}; 
...
let global_ekikan_list = [ 
    ...
    {kiten="v落合"; shuten="中野"; keiyu="東西線"; kyori=2.0; jikan=3}; 
...
let rec get_ekikan_kyori eki1 eki2 lst = match lst with
    [] -> infinity
  | ({kiten = k; shuten = s; keiyu = sen; kyori = km; jikan = t} as first) :: rest ->
...
let koushin p v ekikan_lst =
  List.map (fun q -> match (p, q) with
                       ({namae = p_namae; saitan_kyori = p_saitan; temae_list = p_temae},
                        {namae = q_namae; saitan_kyori = q_saitan; temae_list = q_temae}) ->
                       let kyori = get_ekikan_kyori p_namae q_namae ekikan_lst in
                       if kyori = infinity
                       then q
                       else if p_saitan +. kyori < q_saitan
                       then  {namae = q_namae; saitan_kyori = p_saitan +. kyori; temae_list = q_tamae :: p_temae}
                       else q) v;;

(* テスト *)
let eki1 = {namae="茗荷谷"; saitan_kyori = 0.; temae_list = ["茗荷谷"]} 
let eki2 = {namae="池袋"; saitan_kyori = infinity; temae_list = []} 
let test1 = koushin eki1 [eki1;eki2] global_ekikan_list =
              [eki1;eki2];;
let test2 = koushin eki1 [] global_ekikan_list =
              [];
