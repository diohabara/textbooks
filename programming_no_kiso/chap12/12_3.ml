...
(* 目的 : ekimei_t型のリストを受け取ったら，その駅名からeki_t型のリストを作る *)
(* make_eki_list : ekimei_t list -> eki_t list *)
let rec make_eki_list lst = match lst with
    [] -> []
  | {kanji = k; kana = oka; romaji = r; shozoku = s} :: rest->
     {namae = k; saitan_kyori = infinity; temae_list = []} :: make_eki_list rest;;

(* 目的 : eki_t型のリストと起点(漢字)を受け取ったら，saitan_kyoriが0，temae_listが始点のeki_t型リストを返す *)
(* shorika : eki_t list -> string -> eki_t list *)
let rec shokika lst kiten = match lst with
    [] -> []
  | ({namae = k; saitan_kyori = s; temae_list = t} as first) :: rest ->
     if k = kiten
     then {namae = kiten; saitan_kyori = 0.0; temae_list = [k]} :: shokika rest kiten
     else {namae = k; saitan_kyori = infinity; temae_list = []} :: shokika rest kiten;;
(* テスト *)
let eki_list = [ 
