...
(* ローマ字の英名を2つ受け取り，その間の距離を調べ，
 * 繋がっているときは「A駅からB駅まではxkmです」という文字列を返し，
 * 繋がっていないときは「A駅とB駅は繋がっていません」という文字列を返す *)
(* kyori_wo_hyoji : string -> string -> string *)
let kyori_wo_hyoji eki1 eki2 =
  let eki_a = romaji_to_kanji eki1 global_ekimei_list in
  let eki_b = romaji_to_kanji eki2 global_ekimei_list in
  let kyori = get_ekikan_kyori eki_a eki_b global_ekikan_list in
  if kyori <> infinity
  then eki_a ^ "駅から" ^ eki_b ^ "駅までは" ^ string_of_float kyori ^ "kmです"
  else eki_a ^ "駅と" ^ eki_b ^ "駅は繋がっていません";;

(* テスト *)
let test1 = kyori_wo_hyoji "myogadani" "shinotsuka" = "茗荷谷駅から新大塚駅までは1.2kmです";;
let test2 = kyori_wo_hyoji "omotesandou" "hibiya" = "表参道駅と日比谷駅は繋がっていません";;
kyori_wo_hyoji "myogadani" "shinotsuka";;
kyori_wo_hyoji "omotesandou" "hibiya";;
