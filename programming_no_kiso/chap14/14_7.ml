...
(* 目的 : 直前に確定した駅pと未確定の駅qを受け取り，直接繋がっているかを調べ，繋がっていたらqの最短距離と手前リストを必要に応じて更新する．繋がっていなかったら元のqをそのまま返す *)
...
let koushin1 p q = match (p, q) with
    ...
    then  {namae = q_namae; saitan_kyori = p_saitan +. kyori; temae_list = q_namae :: p_temae}
    else q;;

(* 目的 : 直前に確定した駅pと未確定の駅のリストvを受け取ったら，必要な更新処理後に未確定の駅のリストを返す *)
...
let test1 = koushin eki1 [eki1;eki2] =
              [eki1;eki2];;
let test2 = koushin eki1 [] =
              [];
